<template>
  <NavTemplate/>
  <div class="container">
    <div class="top-line d-flex align-items-center flex-wrap">
      <div class="left d-flex flex-wrap">
        <div class="click-btn d-flex justify-content-center align-items-center add-btn" @click="open_pop_up.add=true">
          <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
               viewBox="0 0 490 490" style="enable-background:new 0 0 490 490;" xml:space="preserve">
            <polygon points="222.031,490 267.969,490 267.969,267.969 490,267.969 490,222.031 267.969,222.031 267.969,0 222.031,0
              222.031,222.031 0,222.031 0,267.969 222.031,267.969 "/>
          </svg>
        </div>
        <div class="click-btn d-flex justify-content-center align-items-center download-btn">
          <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
               viewBox="0 0 325 325" style="enable-background:new 0 0 325 325;" xml:space="preserve">
          <g id="XMLID_786_">
            <g>
              <g>
                <path d="M260.232,83.385h-54.253V2.5h-86.957v80.885H64.769L162.5,214.211L260.232,83.385z M139.021,103.385V22.5h46.957v80.885
                  h34.349L162.5,180.793l-57.827-77.408H139.021z"/>
                <path d="M260,182.5v70H65v-70H0v140h325v-140H260z M305,302.5H20v-100h25v70h235v-70h25V302.5z"/>
              </g>
            </g>
          </g>
        </svg>
        </div>
        <div class="click-btn d-flex justify-content-center align-items-center delete-btn" @click="deleteItem();">
          <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 315 315" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 315 315"><g><path d="m256.774,23.942h-64.836v-6.465c0-9.636-7.744-17.477-17.263-17.477h-34.348c-9.521,0-17.266,7.841-17.266,17.478v6.465h-64.835c-9.619,0-17.445,7.76-17.445,17.297v11.429c0,7.168 4.42,13.33 10.698,15.951 1.989,39.623 13.5,231.193 14.018,239.801 0.222,3.696 3.284,6.58 6.987,6.58h170.033c3.703,0 6.766-2.884 6.987-6.58 0.518-8.607 12.028-200.178 14.018-239.801 6.278-2.621 10.698-8.783 10.698-15.951v-11.43c5.68434e-14-9.537-7.826-17.297-17.446-17.297zm-119.713-6.464c0-1.918 1.465-3.478 3.266-3.478h34.348c1.8,0 3.264,1.56 3.264,3.478v6.465h-40.877v-6.465zm-82.282,23.761c0-1.818 1.546-3.297 3.445-3.297h198.549c1.899,0 3.445,1.478 3.445,3.297v11.429c0,1.819-1.546,3.299-3.445,3.299h-198.548c-1.899,0-3.445-1.479-3.445-3.299v-11.429zm181.143,259.761h-156.848c-2.055-34.247-11.479-191.674-13.51-231.033h183.867c-2.031,39.359-11.454,196.786-13.509,231.033z"></path><path d="m157.5,95.125c-3.866,0-7,3.134-7,7v176.109c0,3.866 3.134,7 7,7 3.866,0 7-3.134 7-7v-176.109c0-3.866-3.134-7-7-7z"></path><path d="m110.2,102.04c-0.202-3.86-3.507-6.837-7.355-6.625-3.86,0.201-6.827,3.494-6.625,7.355l9.182,175.829c0.195,3.736 3.285,6.635 6.984,6.635 0.123,0 0.247-0.003 0.371-0.01 3.86-0.201 6.827-3.494 6.625-7.355l-9.182-175.829z"></path><path d="m212.155,95.415c-3.899-0.223-7.153,2.764-7.355,6.625l-9.184,175.829c-0.202,3.861 2.765,7.154 6.625,7.355 0.125,0.007 0.248,0.01 0.371,0.01 3.698,0 6.789-2.898 6.984-6.635l9.184-175.829c0.202-3.861-2.764-7.154-6.625-7.355z"></path></g></svg>
        </div>
      </div>
      <div class="right">
        <form class="d-flex align-items-center form-find">
          <div>
            <input type="text" class="form-control" placeholder="Введите название">
          </div>
          <button type="submit" class="btn btn-primary">
            <svg width="12px" height="12px" viewBox="0 0 12 12" enable-background="new 0 0 12 12" id="Слой_1" version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><path d="M7,0C4.2385864,0,2,2.2385864,2,5c0,1.2001343,0.4402466,2.2866211,1.1450195,3.1483765L0,11.2929688  L0.7070313,12l3.1450806-3.1446533C4.7138062,9.5598755,5.8001099,10,7,10c2.7614136,0,5-2.2385864,5-5S9.7614136,0,7,0z M7,9  C4.7943726,9,3,7.2056274,3,5s1.7943726-4,4-4s4,1.7943726,4,4S9.2056274,9,7,9z"/></svg>
          </button>
        </form>
      </div>
    </div>
  </div>
  <table class="table table-striped mt-5 main-table">
    <thead>
      <tr>
        <th scope="col">#</th>
        <th scope="col">Тип устройства</th>
        <th scope="col">Марка</th>
        <th scope="col">Модель</th>
        <th scope="col">Сервер</th>
        <th scope="col">Порт</th>
      </tr>
    </thead>
    <tbody>
      <tr v-for="(item,idx) in arr_data" @dblclick="fadeChange(idx);" @click="active_elem.all=idx" v-bind:class="[{ 'active-parent': active_elem.all==idx }]">
        <th scope="row">{{item.id}}</th>
        <td>{{types[item.type].name}}</td>
        <td>{{item.brand}}</td>
        <td>{{item.model_t}}</td>
        <td>{{item.server}}</td>
        <td>{{item.port}}</td>
      </tr>
    </tbody>
  </table>
  <transition name="fade">
    <div class="modal fade show" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" v-if="open_pop_up.add">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Добавление Типа Оборудования</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" @click="open_pop_up.add=false"></button>
          </div>
          <div class="modal-body">
            <form>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label">Тип устройства</label>
                <select class="form-select" :value="add_arr.type" @change="add_arr.type=$event.target.value;">
                  <option selected value="">Нет</option>
                  <option v-for="item in types" :value="item.id">{{item.name}}</option>
                </select>
              </div>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label m-0">Марка</label>
                <input class="form-control" :value="add_arr.brand" @input="add_arr.brand=$event.target.value;"/>
              </div>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label m-0">Модель</label>
                <input class="form-control" :value="add_arr.model_t" @input="add_arr.model_t=$event.target.value;"/>
              </div>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label m-0">Сервер</label>
                <input class="form-control" :value="add_arr.server_t" @input="add_arr.server_t=$event.target.value;"/>
              </div>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label m-0">Порт</label>
                <input class="form-control" :value="add_arr.port_t" @input="add_arr.port_t=$event.target.value;"/>
              </div>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label m-0">Примечание</label>
                <textarea class="form-control" :value="add_arr.comment" @input="add_arr.comment=$event.target.value;"/>
              </div>
              <div class="form-list-item d-flex flex-column align-items-start mb-3">
                <label class="form-label">Параметры датчиков</label>
                <div class="flex-buttons d-flex gap-3 align-items-center">
                  <div class="add-button button d-flex align-items-center" @click='add_arr.params.push({params:"",name:"",units:"",min:"",max:"",coef:"",comment:""})'>
                    <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                         viewBox="0 0 490 490" style="enable-background:new 0 0 490 490;" xml:space="preserve">
                    <polygon points="222.031,490 267.969,490 267.969,267.969 490,267.969 490,222.031 267.969,222.031 267.969,0 222.031,0
                      222.031,222.031 0,222.031 0,267.969 222.031,267.969 "/>
                    </svg>
                    <span>Добавить</span>
                  </div>
                  <div class="delete-button button d-flex align-items-center" @click="deleteItemList('params','add');">
                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 315 315" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 315 315">
                      <g>
                        <path d="m256.774,23.942h-64.836v-6.465c0-9.636-7.744-17.477-17.263-17.477h-34.348c-9.521,0-17.266,7.841-17.266,17.478v6.465h-64.835c-9.619,0-17.445,7.76-17.445,17.297v11.429c0,7.168 4.42,13.33 10.698,15.951 1.989,39.623 13.5,231.193 14.018,239.801 0.222,3.696 3.284,6.58 6.987,6.58h170.033c3.703,0 6.766-2.884 6.987-6.58 0.518-8.607 12.028-200.178 14.018-239.801 6.278-2.621 10.698-8.783 10.698-15.951v-11.43c5.68434e-14-9.537-7.826-17.297-17.446-17.297zm-119.713-6.464c0-1.918 1.465-3.478 3.266-3.478h34.348c1.8,0 3.264,1.56 3.264,3.478v6.465h-40.877v-6.465zm-82.282,23.761c0-1.818 1.546-3.297 3.445-3.297h198.549c1.899,0 3.445,1.478 3.445,3.297v11.429c0,1.819-1.546,3.299-3.445,3.299h-198.548c-1.899,0-3.445-1.479-3.445-3.299v-11.429zm181.143,259.761h-156.848c-2.055-34.247-11.479-191.674-13.51-231.033h183.867c-2.031,39.359-11.454,196.786-13.509,231.033z"/>
                        <path d="m157.5,95.125c-3.866,0-7,3.134-7,7v176.109c0,3.866 3.134,7 7,7 3.866,0 7-3.134 7-7v-176.109c0-3.866-3.134-7-7-7z"/>
                        <path d="m110.2,102.04c-0.202-3.86-3.507-6.837-7.355-6.625-3.86,0.201-6.827,3.494-6.625,7.355l9.182,175.829c0.195,3.736 3.285,6.635 6.984,6.635 0.123,0 0.247-0.003 0.371-0.01 3.86-0.201 6.827-3.494 6.625-7.355l-9.182-175.829z"/>
                        <path d="m212.155,95.415c-3.899-0.223-7.153,2.764-7.355,6.625l-9.184,175.829c-0.202,3.861 2.765,7.154 6.625,7.355 0.125,0.007 0.248,0.01 0.371,0.01 3.698,0 6.789-2.898 6.984-6.635l9.184-175.829c0.202-3.861-2.764-7.154-6.625-7.355z"/>
                      </g>
                    </svg>
                    <span>Удалить</span>
                  </div>
                </div>
                <div class="form-table w-100">
                  <table class="table">
                    <thead>
                    <tr>
                      <th scope="col">#</th>
                      <th scope="col">Параметр</th>
                      <th scope="col">Название</th>
                      <th scope="col">Ед. измерения</th>
                      <th scope="col">Ниж. граница</th>
                      <th scope="col">Верх. граница</th>
                      <th scope="col">Коэффициент</th>
                      <th scope="col">Комментарий</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr v-for="(item,idx) in add_arr.params" @click="active_elem.add.params=idx" v-bind:class="[{ 'active': active_elem.add.params==idx }]">
                      <th scope="col">{{idx}}</th>
                      <th scope="col"><input type="text" :value="item.param" @input="item.param=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.name" @input="item.name=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.units" @input="item.units=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.min" @input="item.min=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.max" @input="item.max=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.coef" @input="item.coef=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.comment" @input="item.comment=$event.target.value;"></th>
                    </tr>
                    </tbody>
                  </table>
                </div>
              </div>
            </form>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" @click="open_pop_up.add=false">Закрыть</button>
            <button type="button" class="btn btn-success" @click="addItem();$event.preventDefault();">Добавить</button>
          </div>
        </div>
      </div>
    </div>
  </transition>
  <transition name="fade">
    <div class="modal fade show" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" v-if="open_pop_up.change">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Добавление Типа Оборудования</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" @click="open_pop_up.change=false"></button>
          </div>
          <div class="modal-body">
            <form>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label">Тип устройства</label>
                <select class="form-select" :value="change_arr.type" @change="change_arr.type=$event.target.value;">
                  <option selected value="">Нет</option>
                  <option v-for="item in types" :value="item.id">{{item.name}}</option>
                </select>
              </div>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label m-0">Марка</label>
                <input class="form-control" :value="change_arr.brand" @input="change_arr.brand=$event.target.value;"/>
              </div>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label m-0">Модель</label>
                <input class="form-control" :value="change_arr.model_t" @input="change_arr.model_t=$event.target.value;"/>
              </div>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label m-0">Сервер</label>
                <input class="form-control" :value="change_arr.server_t" @input="change_arr.server_t=$event.target.value;"/>
              </div>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label m-0">Порт</label>
                <input class="form-control" :value="change_arr.port_t" @input="change_arr.port_t=$event.target.value;"/>
              </div>
              <div class="mb-3 d-flex flex-column align-items-start">
                <label class="form-label m-0">Примечание</label>
                <textarea class="form-control" :value="change_arr.comment" @input="change_arr.comment=$event.target.value;"/>
              </div>
              <div class="form-list-item d-flex flex-column align-items-start mb-3">
                <label class="form-label">Параметры датчиков</label>
                <div class="flex-buttons d-flex gap-3 align-items-center">
                  <div class="add-button button d-flex align-items-center" @click='change_arr.params.push({params:"",name:"",units:"",min:"",max:"",coef:"",comment:""})'>
                    <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                         viewBox="0 0 490 490" style="enable-background:new 0 0 490 490;" xml:space="preserve">
                    <polygon points="222.031,490 267.969,490 267.969,267.969 490,267.969 490,222.031 267.969,222.031 267.969,0 222.031,0
                      222.031,222.031 0,222.031 0,267.969 222.031,267.969 "/>
                    </svg>
                    <span>Добавить</span>
                  </div>
                  <div class="delete-button button d-flex align-items-center" @click="deleteItemList('params','change');">
                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 315 315" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 315 315">
                      <g>
                        <path d="m256.774,23.942h-64.836v-6.465c0-9.636-7.744-17.477-17.263-17.477h-34.348c-9.521,0-17.266,7.841-17.266,17.478v6.465h-64.835c-9.619,0-17.445,7.76-17.445,17.297v11.429c0,7.168 4.42,13.33 10.698,15.951 1.989,39.623 13.5,231.193 14.018,239.801 0.222,3.696 3.284,6.58 6.987,6.58h170.033c3.703,0 6.766-2.884 6.987-6.58 0.518-8.607 12.028-200.178 14.018-239.801 6.278-2.621 10.698-8.783 10.698-15.951v-11.43c5.68434e-14-9.537-7.826-17.297-17.446-17.297zm-119.713-6.464c0-1.918 1.465-3.478 3.266-3.478h34.348c1.8,0 3.264,1.56 3.264,3.478v6.465h-40.877v-6.465zm-82.282,23.761c0-1.818 1.546-3.297 3.445-3.297h198.549c1.899,0 3.445,1.478 3.445,3.297v11.429c0,1.819-1.546,3.299-3.445,3.299h-198.548c-1.899,0-3.445-1.479-3.445-3.299v-11.429zm181.143,259.761h-156.848c-2.055-34.247-11.479-191.674-13.51-231.033h183.867c-2.031,39.359-11.454,196.786-13.509,231.033z"/>
                        <path d="m157.5,95.125c-3.866,0-7,3.134-7,7v176.109c0,3.866 3.134,7 7,7 3.866,0 7-3.134 7-7v-176.109c0-3.866-3.134-7-7-7z"/>
                        <path d="m110.2,102.04c-0.202-3.86-3.507-6.837-7.355-6.625-3.86,0.201-6.827,3.494-6.625,7.355l9.182,175.829c0.195,3.736 3.285,6.635 6.984,6.635 0.123,0 0.247-0.003 0.371-0.01 3.86-0.201 6.827-3.494 6.625-7.355l-9.182-175.829z"/>
                        <path d="m212.155,95.415c-3.899-0.223-7.153,2.764-7.355,6.625l-9.184,175.829c-0.202,3.861 2.765,7.154 6.625,7.355 0.125,0.007 0.248,0.01 0.371,0.01 3.698,0 6.789-2.898 6.984-6.635l9.184-175.829c0.202-3.861-2.764-7.154-6.625-7.355z"/>
                      </g>
                    </svg>
                    <span>Удалить</span>
                  </div>
                </div>
                <div class="form-table w-100">
                  <table class="table">
                    <thead>
                    <tr>
                      <th scope="col">#</th>
                      <th scope="col">Параметр</th>
                      <th scope="col">Название</th>
                      <th scope="col">Ед. измерения</th>
                      <th scope="col">Ниж. граница</th>
                      <th scope="col">Верх. граница</th>
                      <th scope="col">Коэффициент</th>
                      <th scope="col">Комментарий</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr v-for="(item,idx) in change_arr.params" @click="active_elem.change.params=idx" v-bind:class="[{ 'active': active_elem.change.params==idx }]">
                      <th scope="col">{{idx}}</th>
                      <th scope="col"><input type="text" :value="item.param" @input="item.param=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.name" @input="item.name=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.units" @input="item.units=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.min" @input="item.min=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.max" @input="item.max=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.coef" @input="item.coef=$event.target.value;"></th>
                      <th scope="col"><input type="text" :value="item.comment" @input="item.comment=$event.target.value;"></th>
                    </tr>
                    </tbody>
                  </table>
                </div>
              </div>
            </form>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" @click="open_pop_up.change=false">Закрыть</button>
            <button type="button" class="btn btn-success" @click="changeItem();$event.preventDefault();">Изменить</button>
          </div>
        </div>
      </div>
    </div>
  </transition>
</template>

<script>
import NavTemplate from '@/js/components/Nav.vue'
import appToast from '@/js/composables/toast-notification'

export default {
  components:{
    NavTemplate
  },
  setup() {
    const { toastInfo, toastSuccess, toastError, toastWarning } = appToast()
    return {
      toastInfo,
      toastSuccess,
      toastError,
      toastWarning,
    }
  },
  data(){
    return{
      arr_data:[],
      open_pop_up:{
        add:false,
        change:false,
      },
      add_arr:{
        type:"",
        brand:"",
        model_t:"",
        server_t:"",
        port_t:"",
        comment:"",
        params:[

        ]
      },
      change_arr:{
        type:"",
        brand:"",
        model_t:"",
        server_t:"",
        port_t:"",
        comment:"",
        params:[

        ]
      },
      active_elem:{
        add:{
          params:0,
        },
        change:{
          params:0,
        },
        all:-1
      },
      types:{},
    }
  },
  methods:{
    addItem(){
      if(this.isEmpty(this.add_arr.type) || this.isEmpty(this.add_arr.brand)){
        this.toastError('Заполните обязательные поля')
        return false;
      }

      let fData = new FormData();
      fData.append("type",this.add_arr.type);
      fData.append("brand",this.add_arr.brand);
      fData.append("model_t",this.add_arr.model_t);
      fData.append("server_t",this.add_arr.server_t);
      fData.append("port_t",this.add_arr.port_t);
      fData.append("comment",this.add_arr.comment);
      fData.append("params",JSON.stringify(this.add_arr.params));

      axios.post('/equipment-type/add',fData)
          .then(response => {
            let data = response.data;
            if(data.result){
              this.toastSuccess('Элемент добавлен')
              let item = data.data;
              item.params=JSON.parse(item.params);
              this.arr_data.unshift(item);
              this.open_pop_up.add=false;
            }
            else{
              this.toastWarning('Что-то пошло не так. Попробуйте позже')
            }
          })
          .catch(() => this.toastWarning('Что-то пошло не так. Попробуйте позже'))
          .finally(() => (this.loading = false));
    },
    changeItem(){
      if(this.isEmpty(this.change_arr.type)){
        this.toastError('Заполните обязательные поля')
        return false;
      }
      if(!this.change_arr.id || this.change_arr.id<=0){
        this.toastWarning('Что-то пошло не так. Попробуйте ещё раз')
        return false;
      }

      let fData = new FormData();
      fData.append("type",this.change_arr.type);
      fData.append("brand",this.change_arr.brand);
      fData.append("model_t",this.change_arr.model_t);
      fData.append("server_t",this.change_arr.server_t);
      fData.append("port_t",this.change_arr.port_t);
      fData.append("comment",this.change_arr.comment);
      fData.append("params",JSON.stringify(this.change_arr.params));
      fData.append("id",JSON.stringify(this.change_arr.id));

      axios.post('/equipment-type/change',fData)
          .then(response => {
            let data = response.data;
            if(data.result){
              this.arr_data[this.change_arr.idx]=this.change_arr;
              this.toastSuccess('Элемент изменён')
            }
            else{
              this.toastWarning('Что-то пошло не так. Попробуйте позже')
            }
          })
          .catch(error => {
            this.toastWarning('Что-то пошло не так. Попробуйте позже')
          })
          .finally(() => (this.loading = false));
    },
    fadeChange(idx){

      let dop_arr = {};
      for(let i in this.arr_data[idx]){
        dop_arr[i]=this.arr_data[idx][i];
      }
      dop_arr['idx']=idx;
      this.change_arr=dop_arr;
      this.open_pop_up.change=true;
    },
    getData(){
      let fData = new FormData();

      axios.post('/equipment-type/get',fData)
          .then(response => {
            let data = response.data;
            if(data.result){
              this.types=data.device_type;
              for(let i in data.data){
                let item = data.data[i];
                console.log(item.params);
                item.params=JSON.parse(item.params);
                this.arr_data.unshift(item);
              }
            }
            else{
              this.toastWarning('Что-то пошло не так. Попробуйте позже')
            }
          })
          .catch(error => {
            this.toastWarning('Что-то пошло не так. Попробуйте позже')
            console.log(error);
          })
          .finally(() => (this.loading = false));
    },
    isEmpty(str){
      if(typeof str == "undefined") return true;
      if(str>=0) return false;
      if(str.length==0 || str=="" || str.trim()=="") return true;
      return false;
    },
    deleteItem(){

      if(!confirm("Вы действительно хотите удалить элемент?")){
        return false;
      }

      if(!this.arr_data[this.active_elem.all]){
        this.toastWarning('Что-то пошло не так. Попробуйте ещё раз')
      }
      let fData = new FormData();
      fData.append("id", this.arr_data[this.active_elem.all].id);

      axios.post('/equipment-type/delete',fData)
          .then(response => {
            let data = response.data;
            if(data.result){
              this.arr_data.splice(this.active_elem.all,1);
              this.toastSuccess('Элемент удалён')
            }
            else{
              this.toastWarning('Что-то пошло не так. Попробуйте позже')
            }
          })
          .catch(error => {
            this.toastWarning('Что-то пошло не так. Попробуйте позже')
            console.log(error);
          })
          .finally(() => (this.loading = false));
    },
    deleteItemList(ev,type){
      if(type=="add"){
        if(ev=="params"){
          if(this.add_arr.params[this.active_elem.add.params]) this.add_arr.params.splice(this.active_elem.add.params,1);
        }
      }
      else{
        if(ev=="params"){
          if(this.change_arr.params[this.active_elem.change.params]) this.change_arr.params.splice(this.active_elem.change.params,1);
        }
      }
    }
  },
  mounted() {
    this.getData();
  }
}
</script>
<style>
.modal{
  display: block;
  background: rgba(0,0,0,0.5);
}
.modal .modal-dialog{
  max-width: 900px;
}
table.main-table tr{
  cursor: pointer;
}
.form-list-item .flex-buttons{

}
.form-list-item .flex-buttons .button svg{
  height: 25px;
  fill: white;
  margin-right: 10px;
  transition: 0.3s;
}
.form-list-item .flex-buttons .button{
  border: 2px solid #4BB543;
  background: #4BB543;
  padding: 8px 15px;
  border-radius: 255px;
  color: white;
  font-size: 16px;
  cursor: pointer;
  transition: 0.3s;
}
.form-list-item .flex-buttons .button.delete-button{
  background: #F20000;
  border: 2px solid #F20000;
}
.form-list-item .flex-buttons .button:hover{
  background: none;
  color: black;
}
.form-list-item .flex-buttons .button:hover svg{
  fill: black;
}
.form-list-item input{
  border: none;
  outline: none;
  text-align: center;
  width: 100%;
  background: none;
}
.form-list-item table tr.active{
  background: rgba(0,0,0,0.1);
}
.main-table .active-parent{
  background: rgba(13, 110, 253,0.3);
}
</style>
